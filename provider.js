...
function AuthProvider(props) {
    ...
    const handleLogin = async (data) => {
        ...
        try{
            ...
            console.warn(data, 'LOGGED_IN')
            setAuthorization(data.successData.session.session_key); //AXIOS AUTHORIZATION HEADER
            dispatch({type: LOGGED_IN, user:data.successData.user}); //DISPATCH TO REDUCER
        }catch (error) {
            throw new Error(error);
        }
    };
...
// HELPERS ===================================
export const setAuthorization = (session_key) => {
    console.warn('session_key', session_key)
    // Apply authorization session_key to every request if logged in
    if (!session_key) delete axios.defaults.headers.common["Authorization"];
    else axios.defaults.headers.common["Authorization"] = `${session_key}`;
};
